...
from semantic_browser.daemon._metrics import MetricsRegistry
...
from semantic_browser.daemon._lease import _LeaseError, _LeaseMixin  # noqa: F401
...
# MetricsRegistry moved to daemon._metrics.py
...
class TransparentBrowserDaemon(_LeaseMixin):
    ...
    def _dispatch(self, method: str, path: str, args: dict[str, Any], req: BaseHTTPRequestHandler | None = None) -> Any:
        """Table-driven dispatch from registered route handlers.

        Routes are registered by submodules in ``daemon/routers/`` via
        ``_register(method, path, handler)``.  Exact matches are looked up
        first; dynamic-path routes (session lease/handoff) fall through to
        regex-based resolution.
        """
        from semantic_browser.daemon.routers import build_route_table, resolve_dynamic_route

        route_table = build_route_table()
        handler = route_table.get((method, path))
        if handler is not None:
            result = handler(self, args, req)
            return result

        # Dynamic path fallback — session lease / handoff / reattach routes
        dynamic = resolve_dynamic_route(method, path)
        if dynamic is not None:
            handler, matched_path = dynamic
            # Expose the raw matched path to handlers that parse session name from it
            args["_dispatch_path"] = matched_path
            result = handler(self, args, req)
            return result
